...
// added by C. Pham
        ...
        if (_board==SX1272Chip) {
            ...
            // comment by C. Pham
	//Changed by Camden config2 = (readRegister(REG_MODEM_CONFIG2));	// Save config2 to check update
	// (config2 >> 4) ---> take out bits 7-4 from REG_MODEM_CONFIG2 (=_spreadingFactor)
...
uint8_t SX1272::setACK()
{
	...
	if( state == 0 )
	{
		...
		ACK.length = 2;		  // length = 0 to show that's an ACK
					...
					if( ACK.length == 2 )
					{
						if( ACK.data[0] == CORRECT_PACKET )
						{
							...
							//#if (SX1272_debug_mode > 0)
                                ...
                                printf("%d\n", _rcv_snr_in_ack);		// Printing ACK payload
								printf(" ##\n");
								printf("\n");
							//#endif
						}
